...
class Score(object):
    ...
    def create_digit_list(self):
        """Crea el grupo de imágenes según la puntuación recibida."""
        self.digit_list = []
    ...
    def update(self, score_list, level_info):
        """Actualizaciones de movimiento de puntuación."""
        for number in self.digit_list:
            number.rect.y += self.y_vel

        if score_list:
            # Utiliza un hilo para imprimir el mensaje cuando cambia el puntaje
            print_thread = threading.Thread(target=self.print_score_change, args=(score_list,))
            print_thread.start()
    ...
    def print_score_change(self, score_list):
        """Imprime el mensaje cuando cambia el puntaje"""
        print(f"Puntaje cambiado a: {self.score_string}, Posición Y: {self.digit_list[0].rect.y}")
    ...
    def check_to_delete_floating_scores(self, score_list, level_info):
        """Compruebe si las puntuaciones deben ser eliminadas"""
