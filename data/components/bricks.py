...
class Brick(pg.sprite.Sprite):
    ...
    def handle_states(self):
        """Determina el comportamiento del ladrillo según el estado."""
        if self.state == c.RESTING:
            self.resting()
        elif self.state == c.BUMPED:
            self.bumped()
        elif self.state == c.OPENED:
            # Agrega un hilo para ejecutar la función opened en paralelo
            thread = threading.Thread(target=self.opened)
            thread.start()
            print(f"Hilo abierto para el estado {self.state}")
    ...
    def start_bump(self, score_group):
        """Transiciones de ladrillo al estado BUMPED"""
        self.y_vel = -6
...
class BrickPiece(pg.sprite.Sprite):
    """Pedazos que aparecen al romper los ladrillos"""
    ...
    def check_if_off_screen(self):
        """Eliminar de los grupos de sprites si está fuera de la pantalla"""
